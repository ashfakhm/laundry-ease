...
export function InvoiceForm({ bookingId }: InvoiceFormProps) {
  ...
  function addItem() {
    const finalItemName = selectedItemKey === "Other" ? customItemName : selectedItemKey;
    ...
    setItems([...items, { itemType: finalItemName, quantity, unitPrice, photo }]);
    ...
    try {
      ...
      const res = await fetch(
        `/api/bookings/${bookingId}/invoice`,
        {
          ...
          body: JSON.stringify({ items: payload, total, notes }),
        }
      );
    ...
    <form onSubmit={handleSubmit} className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="p-6 bg-card/80 backdrop-blur-md rounded-2xl border border-border shadow-xl space-y-6 relative overflow-hidden">
        ...
        <div className="flex items-center justify-between">
           ...
           {loadingRates && <span className="loading loading-spinner loading-sm text-primary"></span>}
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-start">
            {/* Item Selection */}
            <div className="md:col-span-4 space-y-2">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1">Item Type</label>
                <select 
            ...
            <div className="md:col-span-2 space-y-2">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1">Qty</label>
                <input
                    type="number"
                    min={1}
                    placeholder="1"
            ...
            {/* Unit Price */}
            <div className="md:col-span-2 space-y-2">
                <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1">Price/Unit</label>
            ...
            <div className="md:col-span-3 space-y-2">
                 <label className="text-xs font-semibold text-muted-foreground uppercase tracking-wider ml-1">Evidence</label>
                 <div className="relative">
                    <input
                        type="file"
                        accept="image/*"
      ...
      <div className="bg-card rounded-2xl border border-border shadow-sm overflow-hidden">
        <div className="px-6 py-4 bg-muted/30 border-b border-border flex justify-between items-center">
            <span className="font-bold text-foreground text-sm tracking-wide">ITEMS ADDED ({items.length})</span>
            <span className="text-xs text-muted-foreground font-medium">Review before submitting</span>
        </div>
        ...
        {items.length === 0 ? (
            <div className="p-12 text-center flex flex-col items-center justify-center gap-3 text-muted-foreground">
                ...
                <p className="text-sm">No items added yet. Start adding items above.</p>
            </div>
        ) : (
            <ul className="divide-y divide-border/50">
                {items.map((item, i) => (
                <li key={i} className="p-4 flex items-center justify-between hover:bg-muted/30 transition-colors group">
                    <div className="flex items-center gap-4">
                         ...
                         <div>
                            ...
                            <p className="text-xs text-muted-foreground font-medium">Qty: {item.quantity} × ₹{item.unitPrice}</p>
                         </div>
                    </div>
      ...
      <div className="flex flex-col md:flex-row gap-6 p-6 bg-primary rounded-2xl shadow-xl text-primary-foreground relative overflow-hidden">
        ...
        <div className="w-full md:w-3/5 z-10">
             <label className="text-xs font-bold uppercase tracking-wider text-primary-foreground/70 mb-2 block">Notes for Customer</label>
             <textarea
        ...
        <div className="w-full md:w-2/5 flex flex-col justify-between items-end text-right z-10">
             <div>
                 <p className="text-sm text-primary-foreground/70 mb-1">Total Estimated Amount</p>
