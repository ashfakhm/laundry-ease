...
- **Message Deletion**
  Users must be able to delete messages. The system must support WhatsApp-style deletion: `for_me` (hidden locally for the user), `for_everyone` (within a configurable 1-hour window, clearing content and showing a placeholder), and `admin_hard_delete` (permanent removal by admins with no trace).
...
| Message deletion                | Users should be able to delete chat messages similar to WhatsApp                                                                 | Implemented — 3-tier deletion: `for_me` (local array), `for_everyone` (1-hour window, broadcasts event, strips content leaving placeholder), `admin_hard_delete` (DB removal). Endpoint checks and real-time socket propagation fully mapped. |
