...
- **Image Cropping**
  The system must include interactive cropping (zoom/pan) for user profile photos to optimize display interfaces and prevent aspect ratio distortion.
...
| User Ban Enforcement              | Active bans must deny login and show reason                                                                                                                                                                                                                                                     | Implemented (`auth.ts` signIn callback checks `blocked_until` & redirects with reason details)                                                                                                                                                                                                                                                                        |
...
| Image cropping | Profile photo uploads should support interactive cropping (zoom/pan) | Implemented — `ImageCropModal` leveraging `react-easy-crop` triggered on client dashboard image selection |
