...
Customers can register on the platform, verify their identity via email and phone OTP, and search for nearby laundry providers using location-based discovery powered by Google Maps. Once a provider is selected, the customer can request a booking, pay a booking fee through Razorpay, and choose from services such as washing, drying, and ironing. After the provider inspects the items and issues a digital invoice, the customer reviews the itemized bill—including item photos, quantities, unit prices, and discounts—while the delivery charge is computed during invoice payment and stored on the resulting order, and either approves and pays or rejects with a reason. Throughout the order lifecycle, the customer can track progress through explicit stages: processing, washing, ironing, ready, out for delivery, and delivered. Delivery is confirmed through a one-time password (OTP), after which the payment enters a 24-hour escrow hold before being released to the provider. Customers can also file complaints within 24 hours of delivery, view their complete order and payment history, and leave star-rated reviews for providers.
...
3. **Digital PDF Invoice Generation:** After pickup and physical inspection of items, providers create printable PDF invoices through the platform using `pdf-lib`. Each invoice includes item names, quantities, unit prices, optional item photos, and discounts. When the seeker proceeds to payment, the system computes any delivery charge and stores it on the resulting order. Customers review the complete invoice and approve or reject it with a reason before any payment is captured.
...
7. **Real-Time Order Chat:** Once an order is created (after invoice payment), seekers and providers can exchange messages in real time via Socket.IO. The chat operates on `order:<id>` rooms with JWT-authenticated connections and DB-verified participant authorization. Messages are persisted in the `order_chats` MongoDB collection and pushed live to all connected participants — no polling required. The system supports text, voice notes, up to 5 photo attachments per message, and message deletion modes for `for_me` and `for_everyone`. Complaint chat extends this with an admin-only hard delete path. The provider messages inbox, order-status page, and seeker order detail page all embed the order chat interface.
...
- **Invoice Generation:** Auto-generated printable PDF invoices featuring itemized breakdowns (name, quantity, unit price), optional item photos, and provider-applied discounts. Delivery charges are computed during invoice payment and stored on the resulting order. Customer review with approve/reject workflow.
...
- **Order Lifecycle Tracking:** Validated state progression (invoiced → processing → washing → ironing → ready → out for delivery → delivered) with server-side transition enforcement. The active order pipeline natively sorts by deadline to prioritize urgent jobs.
