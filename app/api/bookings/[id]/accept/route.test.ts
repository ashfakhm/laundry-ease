...
const {
  mockGetDb,
  mockGetBookingById,
  mockAcceptBookingWithCapacityCheck,
  mockRequireProvider,
  ...
  mockAcceptBookingWithCapacityCheck: vi.fn(),
...
vi.mock("@/lib/db/index", () => ({
  ...
  acceptBookingWithCapacityCheck: mockAcceptBookingWithCapacityCheck,
}));
...
vi.mock("@/lib/razorpay", () => ({
  createRazorpayContact: vi.fn(),
  createRazorpayFundAccount: vi.fn(),
}));
...
describe("PATCH /api/bookings/[id]/accept", () => {
  ...
  it("returns compatibility unauthorized payload for invalid provider id", async () => {
    ...
    const req = new Request("https://laundryease.test/api/bookings/1/accept", {
      method: "PATCH",
    });
  ...
  it("returns compatibility invalid booking id payload", async () => {
    ...
    const req = new Request("https://laundryease.test/api/bookings/bad/accept", {
      method: "PATCH",
    });
    const res = await PATCH(req, {
