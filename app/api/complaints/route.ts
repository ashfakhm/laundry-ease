...
export const POST = withErrorHandling(async (req: Request) => {
  ...
  const { order_id, booking_id, complaint_type, title, description, photos } = result.data;
  ...
  if (order_id) {
    ...
    const foundOrder = await db.collection("orders").findOne({ booking_id: new ObjectId(booking_id) });
    if (!foundOrder) {
...
export const GET = withErrorHandling(async (req: Request) => {
  ...
  // 1. Seeker Logic
  const seekerComplaints = await db.collection("complaints").find({
      seeker_id: userId,
  ...
  const providerComplaints = await db.collection("complaints").find({
      provider_id: userId,
      status: { $in: activeStatuses },
  ...
  const unique = Array.from(new Map(allFound.map(item => [item._id.toString(), item])).values());
