...
export async function POST(
  ...
  try {
    ...
    if (booking.status !== "invoice_created") {
        ...
        return NextResponse.json({ error: "Booking is not in invoice review state" }, { status: 400 });
    }
    ...
    if (!invoice) {
        return NextResponse.json({ error: "Invoice data missing" }, { status: 500 });
    }
    ...
    // Calculate totals
    const subtotal = invoice.subtotal || orderItems.reduce((sum: number, i: any) => sum + i.line_total, 0);
