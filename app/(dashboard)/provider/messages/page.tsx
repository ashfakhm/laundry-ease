  ...
  return (
    <div className="h-[calc(100vh-4rem)] bg-background flex overflow-hidden">
                ...
                >
                  <div className="flex items-start gap-4">
                    <div className={cn(
                       ...
                       selectedChatId === chat._id ? "bg-white/20 text-white" : "bg-gradient-to-br from-primary/10 to-purple-500/10 text-primary"
                    )}>
                      {chat.seeker.name.charAt(0)}
                      <span className={cn(
                         "absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2",
                         selectedChatId === chat._id ? "border-primary bg-green-400" : "border-background bg-green-500"
                      )} />
                    </div>
                    
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline mb-1">
                        <h3 className={cn("font-bold truncate", selectedChatId === chat._id ? "text-white" : "text-foreground")}>
                            {chat.seeker.name}
                        </h3>
                        {chat.lastMessage && (
                           <span className={cn("text-[10px]", selectedChatId === chat._id ? "text-white/70" : "text-muted-foreground")}>
                              {formatDistanceToNow(new Date(chat.lastMessage.timestamp), { addSuffix: false }).replace('about ', '')}
                           </span>
                        )}
                      </div>
                      <p className={cn("text-xs truncate", selectedChatId === chat._id ? "text-white/80" : "text-muted-foreground")}>
                        {chat.lastMessage ? (
                           chat.lastMessage.sender === session?.user?.id ? `You: ${chat.lastMessage.text}` : chat.lastMessage.text
                        ) : (
                      ...
                      <div className="mt-2 flex items-center gap-2">
                         <span className={cn("text-[10px] px-2 py-0.5 rounded-full", selectedChatId === chat._id ? "bg-white/20 text-white" : "bg-muted text-muted-foreground")}>
                            #{chat._id.slice(-6).toUpperCase()}
                         </span>
                         <span className={cn("text-[10px] uppercase font-bold", selectedChatId === chat._id ? "text-white/60" : "text-primary")}>
                            {chat.status}
                         </span>
                      </div>
                    </div>
      ...
      <div className={cn(
         ...
         showMobileList ? "translate-x-full md:translate-x-0 opacity-0 md:opacity-100" : "translate-x-0 opacity-100"
      )}>
        {activeChat ? (
          <>
            {/* Header */}
            <div className="h-16 px-6 border-b border-border bg-background/80 backdrop-blur-md flex items-center justify-between shrink-0">
              <div className="flex items-center gap-3">
                 <button onClick={handleBackToMenu} className="md:hidden p-2 -ml-2 rounded-full hover:bg-muted">
                 ...
                 <div>
                    <h2 className="font-heading font-bold text-foreground leading-tight">{activeChat.seeker.name}</h2>
              ...
              <div className="flex items-center gap-2">
                 {activeChat.seeker.phone && (
                    <a href={`tel:${activeChat.seeker.phone}`} className="p-2.5 rounded-full hover:bg-muted text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="hidden md:flex flex-1 flex-col items-center justify-center text-muted-foreground/50">
             ...
             <h3 className="text-lg font-bold text-foreground/40 font-heading">No Chat Selected</h3>
