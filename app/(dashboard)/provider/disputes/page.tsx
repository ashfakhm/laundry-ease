  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="font-heading text-3xl font-bold tracking-tight">Dispute Cases</h1>
          <p className="text-muted-foreground mt-1">Cases where Admin has requested your input.</p>
        </div>
      </div>
      ...
      {disputes.length === 0 ? (
        <div className="rounded-3xl border border-border/50 bg-card/40 p-12 text-center backdrop-blur-xl">
           ...
           <p className="mt-2 text-muted-foreground">You have no open dispute cases requiring attention.</p>
        </div>
      ) : (
        <div className="grid gap-4">
          {disputes.map((dispute) => (
            <Link 
              ...
              href={`/provider/disputes/${dispute._id}`}
